/*
 *   GRAVITY STRIKE -- weapons/bshrink.cpp
 *
 *
...
bulletShrinker::bulletShrinker(gsPlayer *p,  int puser, int pupgrade)
{
  // bullet init
  type = W_SHRINKER;
  by_player = p->get_sig();
  ...
  xspd = xrot * globals->shoot_speed_bomb*2;
  yspd = yrot * globals->shoot_speed_bomb*2;
...
bulletShrinker::bulletShrinker(long psig, int enr, int px, int py, float pxspd, float pyspd, int pstrength, int pshield, int phull, float puser, int pupgrade)
{
  // bullet init
  type = W_SHRINKER;
  by_player = psig;
...
bulletShrinker::~bulletShrinker()
{
}

void bulletShrinker::move()
{
  Bullet::move();
...
void bulletShrinker::draw(gsMap *m)
{
...
//      clearlist->add(m,x,y,1,1);
      effects->pixels->add(4, x+1, y, 12, 12,
                           ...
                           globals->tbl_yellow, 9, 60*5, 15, 100, 100);
    }
}

int bulletShrinker::search_target(int px, int py, long psig)
{
  int bx, by, ret=0;
  float a=50000, b=0;
...
int bulletShrinker::check_collisions()
{
  ...
  for (int n=0; n < globals->players; n++)
  {
                    ...
                    &newx1, &newy1, &newx2, &newy2, 1 ) )
    {
      ...
      {
        player[n]->set_shrinked(strength);
        gsound->hit_shrinker(player[n]->get_x(), player[n]->get_y());
...
*/               {
  ...
  while (temp_e != NULL)
  {
    if (collision( x-7, y-7, 13, 13,
                   ...
                   &newx1, &newy1, &newx2, &newy2, 1 ) )
    {
      if (temp_e->hit(by_player, by_enemy, 0, x, y, xspd, yspd, W_SHRINKER))
      {
        gsPlayer *p = get_player_by_sig(by_player); 
        if (p) p->inc_bullets_hit(1);
        return 1;
...
int bulletShrinker::check_map_collisions(gsMap *m)
{
