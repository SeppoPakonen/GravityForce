/*
 *   GRAVITY STRIKE -- weapons/bshotgun.cpp
 *
 *
...
bulletShotgun::bulletShotgun(gsPlayer *p,  int puser, int pupgrade, int pspawn)
{
  // bullet init
  type = W_SHOTGUN;
  by_player = p->get_sig();
  user      = puser; 
  spawn     = pspawn; // spawn = 1 -> first bullet (spawns several others), 0 -> (one of the) other bullets
  upgrade   = pupgrade;
  my_by_player = get_player_nr_by_sig(by_player);

  // stat
  if (upgrade == 0) p->inc_bullets_shot(9);
  else if (upgrade == 1) p->inc_bullets_shot(15);
  ...
  xspd = xrot * shoot_speed + p->get_xspd();
  yspd = yrot * shoot_speed - p->get_yspd();
  ...
  if (spawn == 1)
  {
    ...
    a = ao-4; if (a < 0) a = 360 + a; else if (a > 360) a = a - 360;
    mxspd = (sinus[a]) * shoot_speed + p->get_xspd(); myspd = (cosinus[a]) * shoot_speed - p->get_yspd();
    effects->bullets->add(W_SHOTGUN, p->get_sig(), 0, p->get_x()+dx, p->get_y()+dy, mxspd, myspd, strength, hitp_shield, hitp_hull, user, upgrade, 0, 0);
    a = ao-8; if (a < 0) a = 360 + a; else if (a > 360) a = a - 360;
    mxspd = (sinus[a]) * shoot_speed + p->get_xspd(); myspd = (cosinus[a]) * shoot_speed - p->get_yspd();
    effects->bullets->add(W_SHOTGUN, p->get_sig(), 0, p->get_x()+dx, p->get_y()+dy, mxspd, myspd, strength, hitp_shield, hitp_hull, user, upgrade, 0, 0);
    a = ao-12; if (a < 0) a = 360 + a; else if (a > 360) a = a - 360;
    mxspd = (sinus[a]) * shoot_speed + p->get_xspd(); myspd = (cosinus[a]) * shoot_speed - p->get_yspd();
    effects->bullets->add(W_SHOTGUN, p->get_sig(), 0, p->get_x()+dx, p->get_y()+dy, mxspd, myspd, strength, hitp_shield, hitp_hull, user, upgrade, 0, 0);
    a = ao-16; if (a < 0) a = 360 + a; else if (a > 360) a = a - 360;
    mxspd = (sinus[a]) * shoot_speed + p->get_xspd(); myspd = (cosinus[a]) * shoot_speed - p->get_yspd();
    effects->bullets->add(W_SHOTGUN, p->get_sig(), 0, p->get_x()+dx, p->get_y()+dy, mxspd, myspd, strength, hitp_shield, hitp_hull, user, upgrade, 0, 0);
    ...
    if (upgrade == 1)
    {
      ...
      a = ao-6; if (a < 0) a = 360 + a; else if (a > 360) a = a - 360;
      mxspd = (sinus[a]) * shoot_speed + p->get_xspd(); myspd = (cosinus[a]) * shoot_speed - p->get_yspd();
      effects->bullets->add(W_SHOTGUN, p->get_sig(), 0, p->get_x()+dx, p->get_y()+dy, mxspd, myspd, strength, hitp_shield, hitp_hull, user, upgrade, 0, 0);
      a = ao-14; if (a < 0) a = 360 + a; else if (a > 360) a = a - 360;
      mxspd = (sinus[a]) * shoot_speed + p->get_xspd(); myspd = (cosinus[a]) * shoot_speed - p->get_yspd();
      effects->bullets->add(W_SHOTGUN, p->get_sig(), 0, p->get_x()+dx, p->get_y()+dy, mxspd, myspd, strength, hitp_shield, hitp_hull, user, upgrade, 0, 0);
      a = ao-18; if (a < 0) a = 360 + a; else if (a > 360) a = a - 360;
      mxspd = (sinus[a]) * shoot_speed + p->get_xspd(); myspd = (cosinus[a]) * shoot_speed - p->get_yspd();
      effects->bullets->add(W_SHOTGUN, p->get_sig(), 0, p->get_x()+dx, p->get_y()+dy, mxspd, myspd, strength, hitp_shield, hitp_hull, user, upgrade, 0, 0);
    ...
    if (p->get_recoil())
    {
      p->set_xspd(p->get_xspd() - (xrot*1.7));
      p->set_yspd(p->get_yspd() + (yrot*1.7));
    }
        
    // pixels
    effects->pixels->add( 20, p->get_x()+dx, p->get_y()+dy, 1, 1,
                          0, 0, 120,
                          globals->tbl_white, 9, 60*2, 5,
                          300, 300);
  }
...
bulletShotgun::bulletShotgun(long psig, int enr, int px, int py, float pxspd, float pyspd, float pstrength, float pshield, float phull, float puser, int pupgrade, int pspawn)
{
 // bullet init
  type = W_SHOTGUN;
  by_player = psig;
  ...
  // one bullet to spawn them all (0)
  // or spawned bullet? (1)
  if (mode == 0)
  {
//    effects->bullets->add(W_SHOTGUN, p->get_sig(), 0, p->get_x()+dx2, p->get_y()+dy2, xspd, yspd, strength, hitp_shield, hitp_hull, 1, 0);
  }
  
}

bulletShotgun::~bulletShotgun()
{
}

void bulletShotgun::move()
{
  Bullet::move();
...
void bulletShotgun::move(int no_trail)
{
  Bullet::move();
...
void bulletShotgun::draw(gsMap *m)
{
...
void bulletShotgun::draw(BITMAP *m)
{
...
int bulletShotgun::check_collisions()
{
  ...
  while (temp_e != NULL)
  {
                   ...
                   &newx1, &newy1, &newx2, &newy2, 1) )
    {
      ...
      if (temp_e->hit(by_player, by_enemy, strength, x, y, xspd, yspd, W_SHOTGUN))
        return 1;
    }
                     ...
                     temp_bu->get_type() == W_BUMPER ) )
  {
                   ...
                   &newx1, &newy1, &newx2, &newy2, 1 ) )
    {
      ...
      switch (temp_bu->get_type())
      {
        case W_ROCKET : temp_bu->hit(3); break;
        case W_PROBE  : temp_bu->hit(1); break;
        case W_MINE   : temp_bu->hit(1); break;
        case W_BUMPER : temp_bu->hit(2); break;
      }
      return 1;
    }
...
int bulletShotgun::check_map_collisions(gsMap *m)
{
