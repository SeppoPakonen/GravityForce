/*
 *   GRAVITY STRIKE -- menu/m2pstats.cpp
 *
 *
...
long int m2PStats::calc_score(int pl)
{
  int score = 0;
...
void m2PStats::init(int n)
{
  if (!globals->use_user_stats) init_normal(n);
  else init_user(n);
}

void m2PStats::init_normal(int scatter)
{
  char tmp[50];
  ...
  score[1] = calc_score(1);

  // back
  button[M_2PSTATS_OKAY] = new mMainButton();
  if (scatter) button[M_2PSTATS_OKAY]->create(245, 430, 245, 500, 150, 30);
  else button[M_2PSTATS_OKAY]->create(245, 430, 0, 0, 150, 30);
  button[M_2PSTATS_OKAY]->set_text(language->get_string(T_MENU_OK));

  // caption
  button[M_2PSTATS_CAPTION] = new mNormalText();
  button[M_2PSTATS_CAPTION]->create(286, 64, 800, 64);
  button[M_2PSTATS_CAPTION]->set_int(FONT_BANK);
  button[M_2PSTATS_CAPTION]->set_text(language->get_string(T_STATS_CAP_2PLAYER));
  button[M_2PSTATS_CAPTION]->set_int(globals->col_yellow);
  button[M_2PSTATS_CAPTION]->set_flag(MC_INACTIVE);
  ...
  if (globals->game_mode == GAME_DOGFIGHT)
  {
    ...
    f[0][2].x = 13+tx; f[0][2].y = 205+ty;
    f[0][3].x = 13+tx; f[0][3].y = 235+ty;
    f[0][4].x = 13+tx; f[0][4].y = 265+ty;
    f[0][5].x = 13+tx; f[0][5].y = 295+ty;
    f[0][6].x = 30+tx; f[0][6].y = 345+ty;

    // top row
    f[1][0].x = 280+tx; f[1][0].y = 120+ty;
    f[2][0].x = 430+tx; f[2][0].y = 120+ty;
    ...
    button[button_count]->set_int(FONT_ICE);
//    strcpy(tmp, language->get_string(T_STATS_PLAYER)); strcat(tmp, " 1");
    ...
    button[button_count] = new mNormalText();
    button[button_count]->create(f[2][0].x, f[2][0].y, 640+f[2][0].x, f[2][0].y);
    button[button_count]->set_int(FONT_ICE);
//    strcpy(tmp, language->get_string(T_STATS_PLAYER)); strcat(tmp, " 2");
    strcpy(tmp, globals->pplayer[1].pname);
    button[button_count]->set_text(tmp);
    button[button_count]->set_int(globals->col_green);
    button[button_count]->set_flag(MC_INACTIVE);
...
void m2PStats::init_user(int scatter)
{
  int n, n2;
  char tmp[50];
  ...
  // top row
  f[1][0].x = 280+tx; f[1][0].y = 110+ty;
  f[2][0].x = 430+tx; f[2][0].y = 110+ty;
  ...
  for (int pl=0; pl < 2; pl++)
  {
    ...
    {
      button[button_count] = new mNormalText();
      button[button_count]->create(f[pl+1][n+1].x, f[pl+1][n+1].y, 640+f[pl+1][n+1].x, f[pl+1][n+1].y);
      button[button_count]->set_int(datafont);
...
int m2PStats::handle_controls()
{
  int quit = 0;

  // back
  if ((key[KEY_ESC] && esc_pressed == 0) || button[M_2PSTATS_OKAY]->get_flags() & MC_PUSHED)
  {
    esc_pressed = 1;
    gsound->menu_select();
    switch_to_menu = globals->last_menu;
    button[M_2PSTATS_OKAY]->set_flag(MC_INVISIBLE);
    button[M_2PSTATS_OKAY]->set_flag(MC_INACTIVE);
    button[M_2PSTATS_OKAY]->del_flag(MC_PUSHED);
    button[M_2PSTATS_OKAY]->do_explosion();
    move_out();
  }
...
void m2PStats::move_out()
{
  for (int n=1; n < button_count; n++)
  {
    button[n]->set_dest_y( button[n]->get_y() + 400 );
  }
  button[M_2PSTATS_CAPTION]->set_dest_x( button[M_2PSTATS_CAPTION]->get_x() + 400 );
