/*
 *   GRAVITY STRIKE -- enemies/etank1.cpp
 *
 *
...
enemyTank1::enemyTank1(int px, int py, float penemy, float pfreq, float pradius, float pspeed, float pstrength, float weaptype)
{
  type          = E_TANK_MOBILETOWER;
...
void enemyTank1::init()
{
  Enemy *packed_enemy = enemies->add(3500+nr, (int)etype, x, y, psig, user[0], user[1], user[2], user[3], user[4]);
...
enemyTank1::~enemyTank1()
{
}

void enemyTank1::move()
{
  Enemy::move();
  if (freezed)
  {
...
void enemyTank1::shoot()
{
}

int enemyTank1::hit(long fromplayer, int fromenemy, float n, int fx, int fy, float fxspd, float fyspd, int weapon)
{
...
void enemyTank1::set_maxhit(int n)
{
  maxhit = n;
