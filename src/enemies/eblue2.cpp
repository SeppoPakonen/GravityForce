/*
 *   GRAVITY STRIKE -- enemies/eblue2.cpp
 *
 *
...
enemyShipBlue2::enemyShipBlue2(int px, int py, float pfreq, float pradius, float pspeed, float pstrength, float targplayer, float weaptype)
{
  type          = E_SHIP_BLUE2;
...
enemyShipBlue2::~enemyShipBlue2()
{
}

void enemyShipBlue2::move()
{
  Enemy::move();
  if (freezed)
  {
...
void enemyShipBlue2::shoot()
{
  if (globals->do_all_hooks || do_hooks)
    script->do_hook_enemy_shoot(glob_lua, this);
  if (get_dont_continue()) return;

  int px,py, pxpos, pypos;
  Bullet *tb[6] = { NULL };

  if (bullet_type == W_SINGLE)
  {
    px = (int)x+w/2; py = (int)y;
    tb[0] = effects->bullets->add(W_SINGLE, psig, nr, px, py,    0,    3,  bullet_strength, 0, 0);
    tb[1] = effects->bullets->add(W_SINGLE, psig, nr, px, py,  1.5,  1.5,  bullet_strength, 0, 0);
    tb[2] = effects->bullets->add(W_SINGLE, psig, nr, px, py, -1.5,  1.5,  bullet_strength, 0, 0);
  
    px = (int)x+w/2; py = (int)y+h;
    tb[3] = effects->bullets->add(W_SINGLE, psig, nr, px, py,    0,   -3,  bullet_strength, 0, 0);
    tb[4] = effects->bullets->add(W_SINGLE, psig, nr, px, py,  1.5, -1.5,  bullet_strength, 0, 0);
    tb[5] = effects->bullets->add(W_SINGLE, psig, nr, px, py, -1.5, -1.5,  bullet_strength, 0, 0);
  
    for (int n=0; n < 6; n++)
      tb[n]->set_ttl(60);
  
    gsound->play_sound(SOUND_SHOOT_SINGLE, px, py);
  }
  else
  {
    pxpos = (int)x+w/2; pypos = (int)y+h/2;
    ...
    effects->bullets->add(bullet_type, psig, nr, pxpos, pypos, pxspd, pyspd, bullet_strength, bullet_hitp_shield, bullet_hitp_shield, 0, 0);
  }
}

int enemyShipBlue2::hit(long fromplayer, int fromenemy, float n, int fx, int fy, float fxspd, float fyspd, int weapon)
{
  ...
  effects->pixels->add(15, fx, fy, 0, 0, -fxspd/4, fyspd/4, 50, globals->tbl_blue, 10, 60*3, 30);

  return 1;
}


void enemyShipBlue2::dead()
{
  if (globals->do_all_hooks || do_hooks)
  ...
  for (int n=0; n < 10/div; n++)
  {
    ...
    effects->pixels->add(120/div, x+gsrand()%w, y+gsrand()%h, 10, 10, 2-gsrand()%4, 2-gsrand()%4, 50, globals->tbl_blue, 10, 60*5, 30);
